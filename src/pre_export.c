...
//YA NO DA SEGFAULT PERO HABRIA QUE MIRAR UNA VEZ ESTÉ ESTO CONECTADO CONN READLINE
//SI FUNCIONA BIEN A LA HORA DE AÑADIR VARIABLES DE ENTORNO QUE YA EXISTEN, SUSTITUIRLAS POR LA QUE YA EXISTE
...
char **realloc_add_pre_export_list(t_env *env, char *line)
{
    ...
    while (i < env->pre_export_elements + 1)
    {
        if (check-- > 0 && env->pre_export[i])
        {
            ...
            if (!temp[i])
                return (ft_free_error_arr(temp, i), ft_free_env(env->pre_export), NULL); //Aqui habria que liberar pre_export en caso de q exista
        }
        else
        {
            temp[i] = ft_strdup(line);
            if (!temp[i])
                return (ft_free_error_arr(temp, i), ft_free_env(env->pre_export), NULL); // Aqui igual
        }
        i++;
    }
    temp[i] = NULL;
