...
// 	tree = *root;
// 	if (token->type >= REDIR_OUT && (*tree)->right == NULL)
// 		(*tree)->right = new_leaf(token);
// 	else if (token->type >= REDIR_OUT && (*tree)->right->content->type >= HEREDOC)
// 	{
// 		aux_leaf = (*tree)->right;
// 		(*tree)->right = new_leaf(token);
// 		((*tree)->right)->right = aux_leaf;
// 	}
// 	else if ((token->type >= HEREDOC && token->type < REDIR_OUT) && (*tree)->left == NULL)
// 		(*tree)->left = new_leaf(token);
// 	else if (token->type >= HEREDOC && (token->type >= HEREDOC && token->type < REDIR_OUT))
// 	{
// 		aux_leaf = (*tree)->left;
// 		(*tree)->left = new_leaf(token);
// 		((*tree)->left)->left = aux_leaf;
// 	}
// 	else
// 	{
// 		aux_leaf = *tree;
...
// 	// 	return ;
// 	aux_leaf = *tree;
// 	while ((aux_leaf->content->type >= PIPE && aux_leaf->content->type <= PARENT_CL) && aux_leaf->right)
// 		aux_leaf = aux_leaf->right;
// 	if ((*tree)->right == NULL)
// 		(*tree)->right = new_leaf(token);
// 		return ;
